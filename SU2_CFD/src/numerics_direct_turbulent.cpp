...
<<<<<<< HEAD
    ...
    if (transition)
		  Destruction *= max(min(intermittency,0.1),1.0);
    ...
    production_jacobian_contribution = cb1*(TurbVar_i[0]*dShat+Shat)*Volume;
    if (transition) production_jacobian_contribution *= gamma_eff;
    ...
    destruction_jacobian_contribution = cw1*(dfw*TurbVar_i[0] +	2.*fw)*TurbVar_i[0]/dist_i_2*Volume;
    if (transition) destruction_jacobian_contribution *= max(min(intermittency,0.1),1.0);
        
    val_Jacobian_i[0][0] = production_jacobian_contribution - destruction_jacobian_contribution;
	}
